...
void paritosh() {
    ...
    ll full = p / total;       // full playlists
    ll rem = p % total;        // leftover needed
    ...
    // Duplicate array for circular subarray
    vector<ll> b(2 * n);
    for (int i = 0; i < 2 * n; i++) b[i] = arr[i % n];

    ll ans_len = LLONG_MAX, start = 0;
    ll sum = 0, j = 0;
