...
// push O(1), pop O(1) ,min O(1),max O(1), sliding window O(n)
ll gcd(ll a, ll b) { return b==0 ? a : gcd(b,a%b); }

struct stack{
    vector<ll> s, sgcd={0}; // sgcd[i] = gcd of stack from bottom to i
    void push(ll x){
        s.push_back(x);
        sgcd.push_back(sgcd.back()==0 ? x : gcd(sgcd.back(), x));
    }
    ll pop(){
        ll res = s.back();
        s.pop_back();
