...
bool isPresent(vl& arr,mll& mp,ll& contastOfarrayA){
    // mll mp;
    REP(i,0,arr.size()){
        ll val = arr[i]-contastOfarrayA;
...
void paritosh() {
    ...
    vl arr(n);
    mll mp;
    ll contastOfarrayA=0;
    REP(i,0,n){
        cin>> arr[i];
        mp[arr[i]]++;
    }
    REP(i,1,n){
        contastOfarrayA+=abs(arr[i-1]-arr[i]);
    }
    if(contastOfarrayA==0){
        cout<<1<<endl;
        return;
    }
    bool flag=isPresent(arr,mp,contastOfarrayA);
    if(flag==true){
        cout<<2<<endl;
    }
    else{
