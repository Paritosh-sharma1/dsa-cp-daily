/*
    ...
    Implementation of AP on Prefix Sum
*/
#include <bits/stdc++.h>
using namespace std;
...
void paritosh() {
    ...
    vl prefA(n+1,0),prefB(n+1,0);
   
    for (ll i = 1; i <= n; i++) {
        ll x = (arr[i - 1] % MOD + MOD) % MOD;

        prefA[i] = (prefA[i - 1] + x * i) % MOD;
        prefB[i] = (prefB[i - 1] + x) % MOD;
    }
    ...
    REP(i,0,q){
        ...
        ll val = (prefA[r]-prefA[l-1]+MOD)%MOD;
        debug(val);
        ll val2 = (prefB[r]-prefB[l-1]+MOD)%MOD;
        debug(val2);
        cout<<(val+((1-l)*val2)%MOD+MOD)%MOD<<endl;
    }
}
